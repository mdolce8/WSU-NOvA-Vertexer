...
while idx < (len(test_mode)):
    ...
    if test_mode[idx] == 0.0: # quasi-elastic
        ...
        coh_true_vtxx.append(testtruevtxx[idx])
        coh_reco_vtxx.append(testrecovtxx[idx])
        ...
        mec_true_vtxx.append(testtruevtxx[idx])
        mec_reco_vtxx.append(testrecovtxx[idx])
    idx+=1
print('\nJob complete.')
...
qe_reco_vtxx,res_reco_vtxx,dis_reco_vtxx,coh_reco_vtxx,mec_reco_vtxx=([] for i in range(5))
qe_reco_true_vtxx,res_reco_true_vtxx,dis_reco_true_vtxx,coh_reco_true_vtxx,mec_reco_true_vtxx=([] for i in range(5))
x=idx=0
...
while idx < (len(train_mode)):
    ...
    if train_mode[idx] == 0.0: # quasi-elastic
        ...
        coh_reco_vtxx.append(recovtxx[idx])
        coh_reco_true_vtxx.append(truevtxx[idx])
    elif train_model[idx] == 10.0: #Mec
        mec_reco_vtxx.append(recovtxx[idx])
        mec_reco_true_vtxx.append(truevtxx[idx])
    idx+=1
print('\nJob complete.')
...
model_regCNN.save('x_model.h5')

# Save as .pb file
tf.saved_model.save(model_regCNN, 'x_model.pb')
